...
for rr = 1:size(C_df,1)
    ...
    for ii = 1:length(fieldnames(pips)) % loop over types of ITI
        ...
        for pp = 1:length(p) % loop over high/low rate epochs
            ...
            relevantFrameInd = frameInd(p(pp).ind);
            outA = zeros(1,length(1:10:length(relevantPips)-w*vec.sr+1));
            outP = zeros(1,length(1:10:length(relevantPips)-w*vec.sr+1));
            tmpc = 0;
            for ww = 1:10:length(relevantPips)-w*vec.sr+1
                tmpc = tmpc+1;
                tmp = ww:ww+w*vec.sr-1;
                outA(tmpc) = mean(S_df(rr,unique(relevantFrameInd(tmp))))/w;
                outP(tmpc) = sum(relevantPips(tmp))/w;
            end
        
            % bin the pip/s bins into X bins
            [~,pipbins] = histc(outP,pipSecBinVec); % assign bins given by pipSecBinVec
            nEventsCell{cc} = accumarray(pipbins',outA',[],@(x) {x}); % given cell contains all time bins that have a certain pips/s
            ...
            p1(rr,cc) = kruskalwallis(outA,pipbins,'off'); 
            cc = cc+1;
        end
    end % end D1/D2
